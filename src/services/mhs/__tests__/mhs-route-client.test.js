...
describe('mhs-route-client', () => {
  ...
  it('should call mhs with correct interaction id', async () => {
    const interactionId = 'urn:nhs:names:services:gp2gp:RCMR_IN010000UK05';
    axios.get.mockResolvedValue({ status: 200, data: { uniqueIdentifier: [expectedAsid] } });
    await getPracticeAsid(odsCode);

    expect(axios.get).toHaveBeenCalledWith(
      expect.anything(),
      expect.objectContaining({ params: expect.objectContaining({ 'service-id': interactionId }) })
    );
  });
  ...
  it("should throw if the call to MHS doesn't return 200", async () => {
    ...
    await expect(getPracticeAsid(odsCode)).rejects.toThrow(
